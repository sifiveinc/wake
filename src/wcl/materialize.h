...
result<CopyResult, posix_error_t> materialize_regular_file_at(
    ...
    time_t mtime_sec, long mtime_nsec, bool attempt_reflink = true);
...
result<CopyResult, posix_error_t> materialize_regular_file(
    ...
    long mtime_nsec, bool attempt_reflink = true);
...
result<bool, posix_error_t> materialize_symlink(
    const std::string& destination, const std::string& target, time_t mtime_sec, long mtime_nsec);
